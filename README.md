...
Compare revisions with `gitcity owner/repo --compare main..feature`, or press `B` and enter `before..after` (default `HEAD~1..HEAD`). Use left/right arrows or click the slider to reveal either version. Green `+`, coral `−`, and gold `~` mark added, deleted, and changed files. Select a file for exact revision contents and the two-version diff. Press `B` again to return. `--at <commit-or-branch>` starts exploration at a specific revision.
